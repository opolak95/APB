...
// Slash command and modal submission
client.on(Events.InteractionCreate, async interaction => {
  if (interaction.isChatInputCommand() && interaction.commandName === 'create') {
    try {
      ...
      console.error('Chyba při zobrazení modalu:', err);
    }
  }
  ...
  if (interaction.isModalSubmit() && interaction.customId === 'event_details') {
    try {
      ...
      // Hardcoded event preset for test – in future use session
      const preset = eventPresets['ss'];
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          { name: '🗓️ Datum', value: date, inline: true },
      ...
      if (!interaction.replied) interaction.reply({ content: 'Došlo k chybě při vytváření eventu.', ephemeral: true });
    }
  }
  ...
  if (interaction.isButton()) {
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(preset.description)
        .addFields(
          { name: '🗓️ Datum', value: activeEvent.meta.date, inline: true },
